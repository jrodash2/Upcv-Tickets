...
def corregir_existentes(apps, schema_editor):
    Requisito = apps.get_model("gestion_empleados", "CatalogoRequisito")
    for orden, (codigo, descripcion, fase) in enumerate(REQUISITOS, 1):
        # UPDATE conserva PK, detalles, revisiones y cumplimiento. Si falta un
        # código no se inventa un segundo catálogo ni se duplica otro registro.
        Requisito.objects.filter(codigo=codigo).update(
...
class Migration(migrations.Migration):
    dependencies = [("gestion_empleados", "0010_nombres_requisitos_fr029")]

    operations = [migrations.RunPython(corregir_existentes, migrations.RunPython.noop)]
