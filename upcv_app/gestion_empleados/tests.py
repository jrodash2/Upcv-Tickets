...
class ProcesoContratacionFlujoTests(TestCase):
    ...
    def test_todas_las_urls_de_gestion_resuelven_vistas_existentes(self):
        from . import views

        patrones = get_resolver().url_patterns
        gestion = next(
            patron for patron in patrones
            if getattr(patron, "namespace", None) == "gestion_empleados"
        )
        for patron in gestion.url_patterns:
            with self.subTest(ruta=str(patron.pattern), nombre=patron.name):
                self.assertTrue(callable(patron.callback))
                self.assertIsNotNone(
                    getattr(views, patron.callback.__name__, None),
                    f"La URL {patron.name} apunta a una vista inexistente.",
                )
    ...
    def test_contrato_creado_y_firmado_no_crean_empleado_hasta_aprobacion(self):
        ...
        self.assertEqual(contrato.firmado_por, self.user)
        self.assertIsNotNone(contrato.fecha_firma)
        self.assertFalse(Empleado.objects.filter(dpi="9000000000026").exists())
